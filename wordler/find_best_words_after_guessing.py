...
def per_word_outcomes(guess_word,known_wrong_positions_initial = {},available_answers_initial = all_answers):
    ...
    for single_outcome in all_outcomes_list:
        ...
        known_wrong_positions,available_answers = \
            calc_remaining_words(known_wrong_positions, available_answers_initial,
...
def per_word_outcomes_wrapper(args):
    ...
    this_word_outcomes = list(per_word_outcomes(guess_word=guess_word,known_wrong_positions_initial=known_wrong_positions_initial,available_answers_initial=available_answers_initial))
...
def generate_all_words_outcomes(known_wrong_positions_initial = {},available_answers_initial=all_answers):
    if multiprocessing_threads is None:
        ...
        for guess_index, guess_word in list(enumerate(all_guesses)):
            this_word_outcomes = per_word_outcomes_wrapper(args=(guess_index, guess_word,known_wrong_positions_initial,available_answers_initial))
            all_outcomes_list.append(this_word_outcomes)

    else:
        all_args = [(guess_index, guess_word,known_wrong_positions_initial,available_answers_initial) for guess_index, guess_word in list(enumerate(all_guesses))]
...
def calc(known_wrong_positions_initial = {},available_answers_initial=all_answers):
    ...
    all_outcomes_list = generate_all_words_outcomes(known_wrong_positions_initial = known_wrong_positions_initial,available_answers_initial=available_answers_initial)
...
def calc_after_guessing(guess_words,guess_results,number_of_results_to_display=100):
    ...
    for i in range(1,len(guess_words)):
        known_wrong_positions_initial, available_answers_initial = \
          calc_remaining_words(known_wrong_positions=known_wrong_positions_initial, available_answers=available_answers_initial,
                                 guess_word=guess_words[i], guess_results=guess_results[i])

    if len(available_answers_initial) > 1:
        calc_outcomes(rerun=True,known_wrong_positions_initial=known_wrong_positions_initial,available_answers_initial=available_answers_initial,number_of_results_to_display=number_of_results_to_display)
    else:
...
def calc_outcomes(rerun=False, number_of_results_to_display=25,known_wrong_positions_initial = {},available_answers_initial=all_answers):
    ...
    if rerun:
        print(known_wrong_positions_initial)
        print(len(available_answers_initial))
        calc(known_wrong_positions_initial = known_wrong_positions_initial,available_answers_initial=available_answers_initial)

    remaining_words_given_outcome = np.load("result_with_guesses.npy")

    average_remaining_list_length = np.sum(remaining_words_given_outcome * remaining_words_given_outcome / len(available_answers_initial),
                                           axis=1)
